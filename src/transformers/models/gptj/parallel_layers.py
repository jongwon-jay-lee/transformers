...
class TensorParallelEmbedding(nn.Embedding):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        # No need to broadcast for input, since by
        # `tie_weights` -> self._tie_or_clone_weights(output_embeddings, self.get_input_embeddings())
        # `self.wte` == `self.lm_head`
        # and self.lm_head = nn.Linear(config.n_embd, config.vocab_size // self.process_group.size())
